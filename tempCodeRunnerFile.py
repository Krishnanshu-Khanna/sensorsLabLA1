...
def things_dodged(counting, highest_score, everything_speed):
	...
	# temp=fnt.render("Temperature: "+str(everything_temp)+ "C",True,green_color)
...
def high_score_update(dodged):
	high_scores = open(os.getcwd()+'\\data/high_score.txt', 'w')
	temperd = str(dodged)
...
def read_temperature_from_csv(file_path):
    """Reads the latest temperature value from a CSV file."""
    try:
        with open(file_path, 'r') as file:
            ...
            for row in reader:
                last_row = row
            if last_row:
                return float(last_row[1])  # Assuming temp is in the second column
    except Exception as e:
        print(f"Error reading CSV: {e}")
    return 0  # Default temperature if reading fails

def draw_meter(temp):
    """Draws the temperature meter with dynamic color."""
    ...
    # Draw meter
    pygame.draw.rect(screen, color, (300, 400, 200, 50))

    # Display text
    temp_text = font.render(f"Temperature: {temp:.1f}°C", True, (255, 255, 255))
    screen.blit(temp_text, (320, 360))
...
def looping_gameplay():
	...
	highest_score_txtfile = open(os.getcwd()+'/data/high_score.txt','r')
